   ...
   public function add_new_admin(){
    ...
    if(isset($_SESSION['user_id'])){  
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if(empty($data['pp'])){
                $data['profile_pic_err']='profile picture cannot be empty';
            
            }
            // else{
            //     $fileExt=explode('.',$_FILES['admin_pro_pic']['name']);
            //     $fileActualExt=strtolower(end($fileExt));
            //     $allowed=array('jpg','jpeg','png');

                
            //     if(!in_array($fileActualExt,$allowed)){
            //         $data['profile_pic_err']='You cannot upload files of this type';

            //     }
            

            //   if($data['admin_pro_pic']['size']>0){
            //     if(uploadImage($data['admin_pro_pic']['tmp_name'],$data['profile_pic_name'],'/upload/user_profile_pics/')){
                          
            //     }else{  
            //     $data['profile_pic_err']='Unsuccessful profile picture uploading';
                
            //     }
            //   }else{
            //     $data[ 'profile_pic_err'] ="profile pic size is empty";
