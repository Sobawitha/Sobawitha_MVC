<?php
    ...
    {
        $this->db->query("SELECT p.order_id, p.quantity, s.created_at, r.product_name, r.price, r.raw_material_image, p.review_status FROM raw_material AS r JOIN seller_order_raw_material AS p ON p.product_id = r.Product_id JOIN seller_orders AS s ON s.order_id = p.order_id WHERE r.product_name LIKE '%$search%' AND s.seller_id=$userId"  );
